...
시뮬레이션에서는 AXI write transaction으로 register를 설정한 뒤, 내부 counter와 readback 값을 함께 확인한다. `PSC=99`이면 100MHz 기준 prescaler tick을 100클럭마다 만들고, `ARR=999`이면 counter가 `0`부터 `999`까지 동작한 뒤 rollover된다.

| 확인 항목 | 봐야 할 신호/값 |
| :--- | :--- |
| control register write | `slv_reg0[0]=cnt_en`, `slv_reg0[1]=intr_en` |
| prescaler 설정 | `PSC=99` |
| auto-reload 설정 | `ARR=999` |
| counter 진행 | `o_cnt` 증가와 `999 -> 0` rollover |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `0x00` | `TIM_CR` | `[0] CNT_EN` | R/W | `0` | counter enable |
| `0x00` | `TIM_CR` | `[1] INTR_EN` | R/W | `0` | interrupt enable |
...
현재 `timer_v1_0_S00_AXI.v`는 4개의 32-bit slave register를 사용한다. Write address decode는 `axi_awaddr[ADDR_LSB+OPT_MEM_ADDR_BITS:ADDR_LSB]` 기준으로 `0x00`, `0x04`, `0x08`, `0x0C`를 구분한다. `CNT` 주소에 write하면 `slv_reg3`에 값이 저장되고, 동시에 `cnt_valid_r`이 1클럭 올라가 TimerCounter의 `counter <= i_cnt` load 동작으로 연결된다. `CNT` read는 저장된 `slv_reg3`가 아니라 현재 counter 값인 `o_cnt`를 반환한다.
...
| Register 구분 | 접근 | 역할 |
| :--- | :--- | :--- |
| Status Register `SR` | Read only | `tx_ready`, `rx_flag` 같은 peripheral 상태 제공 |
| Transmit Data Register `TDR` | Write 중심 | CPU write data를 UART TX로 전달 |
| Receive Data Register `RDR` | Read 중심 | UART RX data 저장, read 시 RX flag clear |
| Control Register `CR` | Read/Write | interrupt enable 등 제어 bit 저장 |
...
| interrupt | `CR` interrupt enable과 status flag 조합 |
...
| data write | `TDR`에 `AA`, `55`, `33`, `44` 순서 write |
...
Interrupt 방식은 RX flag를 계속 읽는 대신 interrupt 발생을 기다린 뒤 `RDR`을 읽는 구조이다. 수업에서는 `FF`, `11`, `22`를 전송 대상으로 두고 interrupt 발생 이후 `RDR`에서 해당 값이 순서대로 읽히는지 확인했다.
...
Timer IP는 `TimerCounter.v`를 별도 Vivado source로 추가하는 방식이 아니라 `timer_v1_0.v` 안에 `TimerCounter` module을 함께 포함하는 구조로 정리했다. 이 방식이면 IP package file list에는 `timer_v1_0.v`, `timer_v1_0_S00_AXI.v`만 남고, TimerCounter는 top 내부에서 함께 합성된다.
...
Vitis application은 기존 stopwatch software 계층을 재사용하는 방향으로 시작했다. 현재 `main.c`는 `StopWatch_Init()` 후 main loop에서 `StopWatch_Execute()`, `FND_Excute()`, `incTick()`, `delay_ms(1)`을 반복하는 polling service routine 구조이다. 이후 Timer interrupt 기반으로 시간 기준을 옮기려면 `common/interrupt`, `HAL/TMR` 계층에 Timer/INTC 초기화와 ISR 처리를 추가해야 한다.

| 계층 | 현재 연결 방향 |
| :--- | :--- |
| `main.c` | application loop와 polling service routine |
...
| `common/interrupt` | INTC 초기화와 ISR 연결 예정 |
| `HAL/TMR` | Timer register 접근 HAL 예정 |
...
- UART RTL 작성 후 TimerCounter와 같은 AXI peripheral에 통합
- UART TX/RX valid, ready, done 계열 신호의 register 연결 방식 결정
- UART register write/read side effect 시뮬레이션
- UART loopback polling test에서 `TDR` write와 `RDR` read 값 일치 확인
- UART interrupt 방식에서 interrupt 발생 후 `RDR` read 확인
