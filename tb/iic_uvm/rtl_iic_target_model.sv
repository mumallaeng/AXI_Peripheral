...
module rtl_iic_target_model (
    ...
    input  logic       ack_addr_en,
    input  logic       ack_data_en,
    output logic       sda_drive_low,
    output logic [7:0] rx_data,
    output logic       rx_valid,
    output logic [7:0] addr_seen,
    output logic       rw_seen,
    output logic [7:0] start_count,
    output logic [7:0] stop_count
);
                    ...
                    TGT_RX_ADDR: begin
                        if (scl_rise) begin
                            ...
                            if (bit_cnt == 3'd7) begin
                                ...
                                rw        <= addr_byte[0];
                                bit_cnt   <= '0;
                                if ((addr_byte[7:1] == own_addr) && ack_addr_en) begin
                                    state <= TGT_ADDR_ACK;
                                end else begin
                                    state <= TGT_IGNORE;
                                end
